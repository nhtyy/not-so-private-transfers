...
contract Deployer is IDeployer {
    ...
    bytes32 constant RELAY_AUTH_TYPEHASH =
        0xbd55b7f7526a00c1ed7ef41c425970ddfb173696d7e3ef728d18ce0020f19c58;
    ...
    function relayDeploy(
        RelayAuthentication calldata relayAuthentication,
        ...
        LibWallet.makeCall(
            wallet,
            relayAuthentication.to,
            relayAuthentication.value,
            relayAuthentication.data
        );

        emit Deployed(
            relayAuthentication.owner,
            relayAuthentication.salt,
            wallet
        );
    ...
    function eip712StructHash(
        RelayAuthentication calldata m
    ) public pure returns (bytes32) {
        bytes memory encodeStruct = abi.encode(
            m.owner,
            m.salt,
            m.to,
            m.value,
